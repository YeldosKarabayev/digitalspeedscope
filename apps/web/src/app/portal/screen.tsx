...
export default function PortalScreen() {
  ...
  React.useEffect(() => {
    ...
    setPointId(portal.pointId);
    setClientIp(portal.clientIp);
    setClientMac(portal.clientMac);
    setContinueUrl(portal.continueUrl);
    setDeviceKey(getOrCreateDeviceKey());
  }, []);

  React.useEffect(() => {
    if (!deviceKey || !pointId) return;
  ...
  function getOrCreateDeviceKey() {
    ...
    document.cookie = `dss_device=${encodeURIComponent(key)}; path=/; max-age=31536000; samesite=lax`;
    return key;
  }
  ...
  async function requestCode() {
    ...
    if (!pointId) {
      setErr("Не удалось определить точку Wi-Fi.");
      return;
    }

    if (!canRequest) {
      setErr("Введите корректный номер Казахстана.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_URL}/portal/request-code`, {
        ...
        body: JSON.stringify({
          ...
          clientMac: clientMac || undefined,
        }),
      });

      if (!res.ok) {
        throw new Error(await readError(res, "Не удалось отправить код"));
      }
  ...
  async function verifyCode() {
    ...
    if (!deviceKey) {
      setErr("Не удалось определить устройство.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readError(res, "Неверный код"));
      }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto grid min-h-screen max-w-[520px] place-items-center px-4 py-10">
        <Card className="w-full rounded-3xl border border-slate-800 bg-slate-950/60 p-6 shadow-sm">
          ...
          {pointId ? (
            <div className="mb-5 text-center text-[11px] text-slate-500">
              Точка: {pointId}
            </div>
          ) : (
            <div className="mb-5">
              <ErrorBox text="Параметр pointId не передан в captive portal URL." />
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <Button
                className={cn(
                  ...
                  (loading || !canRequest || !pointId) && "opacity-90",
                )}
                disabled={loading || !canRequest || !pointId}
                onClick={requestCode}
              >
                {loading ? (
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/20 p-4">
                ...
                <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500">
                  ...
                  >
                    {expiresIn > 0
                      ? `Отправить снова через ${expiresIn}s`
                      : "Отправить снова"}
                  </button>
                </div>
              </div>
          ...
          {step === "success" ? (
            <div className="space-y-4">
              ...
              <div className="text-center text-[11px] text-slate-500">
                Если интернет не открылся — выключите и включите Wi-Fi, затем попробуйте снова.
              </div>
            </div>
          ) : null}
        </Card>
